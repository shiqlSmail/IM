...
public class SendEmailController extends BaseController {
    ...
    public Map<String, Object> sendEmailUrl(@RequestBody @Valid EmailSendParam param) {
        ...
        try {
            ...
            if (isFlag) {
                ...
                map.put("resCode", UserEunms.BIZ_ADMIN_SUCCESS.getResCode());
                map.put("resData", "激活链接已发送你的【"+param.getEmail()+"】邮箱，请注意查收！");
            } else {
                map.put("resMsg", "邮件发送失败");
                map.put("resCode", UserEunms.BIZ_FAIL.getResCode());
                map.put("resData", "");
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Map<String, Object> sendEmailYzm(@RequestBody @Valid EmailSendParam param) {
        long now = System.currentTimeMillis();
        Map<String, Object> map = new HashMap<>();

...
public class FileUploadTool {
    ...
    public FileEntity createFile(MultipartFile multipartFile, HttpServletRequest request,String path) {
        ...
        if (bflag) {
            ...
            // 绝对路径
            String fileNamedirs = path + File.separator + newFileName + fileEnd;
            ...
            String fileDir = path + newFileName + fileEnd;
...
//            boolean flag = false;
            if (this.checkAVIType(fileEnd)) {
                // 设置转换为AVI格式后文件的保存路径
                String codcAviPath = path + File.separator + newFileName + ".avi";
            ...
            if (aviPath != null) {
                ...
                if (this.checkMediaType(fileEnd)) {
                    try {
                        // 设置转换为flv格式后文件的保存路径
                        String codcFilePath = path + File.separator + newFileName + ".flv";
